...
- no-rerank 配置：`config/retrieval_no_rerank.yaml`
- BGE 配置：`config/retrieval_bge_reranker.yaml`
- no-rerank 版本直接保留 hybrid retrieval 顺序，不应用 heuristic、cross-encoder 或 BGE reranking
- BGE 版本在 hybrid candidate retrieval 后用 `BAAI/bge-reranker-v2-m3` 对 query-passage pairs 重新打分
- 两个版本都保持 `retrieval_json` handoff schema 不变，便于 P1/P5 做同接口对比
- 当前只说明可做 ablation，不声称 final benchmark accuracy improvement
...
- no-rerank vs BGE rerank ablation 已提供独立配置，后续可用同一 P1/P5 harness 比较 reranking 对 evidence ordering 的影响。
...
- `config/retrieval_no_rerank.yaml`
