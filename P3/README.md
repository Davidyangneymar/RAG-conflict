...
当前还提供一个可选 BGE reranker 配置：`config/retrieval_bge_reranker.yaml`。它不会替换默认 `config/retrieval.yaml`，也不会改变 P3 -> P1 handoff schema。
...
- BGE reranker 已作为可选 rerank backend 接入；默认 baseline 仍保持不变。
