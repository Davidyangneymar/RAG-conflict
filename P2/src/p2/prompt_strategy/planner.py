...
from ..conflict_typing import TypedSample
...
def build_answer_plan_for_sample(
    typed_sample: TypedSample,
...
def build_answer_plans(
    typed_samples: Iterable[TypedSample],
    input_records: Iterable[InputRecord],
) -> List[AnswerPlan]:
    return [
...
def build_answer_context(typed_sample: TypedSample, input_record: InputRecord) -> AnswerContext:
    ...
    for typed_pair in typed_sample.pair_results:
        ...
        for claim_id in (typed_pair.stance.claim_a_id, typed_pair.stance.claim_b_id):
            ...
            bucket = _cluster_key(evidence.stance_label)
            evidence_clusters[bucket].append(evidence)
...
def _extract_query(input_record: InputRecord) -> str:
...
def _primary_type(typed_sample: TypedSample) -> str:
